...
object Analyzer {
  ...
  /** Detecta las entidades del diccionario que aparecen en el texto dado.
    ...
    *
    * Ejemplo: text = "Scala fue creado en EPFL por Martin Odersky" dictionary =
    * List( ProgrammingLanguage("Scala"), University("EPFL"), Person("Martin
    * Odersky"), Person("Ada Lovelace") ← no aparece en el texto ) resultado =
    * List( ProgrammingLanguage("Scala"), University("EPFL"), Person("Martin
    * Odersky") )
    */
  def detectEntities(
  ...
  def countByType(entities: List[NamedEntity]): Map[String, Int] = {
    entities.groupBy(_.entityType).map { case (entity, list) =>
      entity -> list.length
    }
  }
}
